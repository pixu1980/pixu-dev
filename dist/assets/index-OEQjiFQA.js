...
`;function t(){return`adoptedStyleSheets`in document&&`replaceSync`in CSSStyleSheet.prototype}function n(e,n){if(t()){let e=new CSSStyleSheet;e.replaceSync(n),document.adoptedStyleSheets=[...document.adoptedStyleSheets,e];return}if(document.head.querySelector(`style[data-component="${e}"]`))return;let r=document.createElement(`style`);r.dataset.component=e,r.textContent=n,document.head.append(r)}function r(e,t){!customElements.get(e)&&customElements.define(e,t)}var i=`pixu:accent-color`,a=document.documentElement,o=[{value:`blue`,label:`Blue`,color:`#0072b2`},{value:`green`,label:`Green`,color:`#007a5a`},{value:`orange`,label:`Orange`,color:`#b94700`},{value:`purple`,label:`Purple`,color:`#6d55b8`},{value:`rose`,label:`Rose`,color:`#9b3d73`}],s=0;function c(){try{let e=window.localStorage.getItem(i);return o.some(t=>t.value===e)?e:`blue`}catch{return`blue`}}function l(e){try{window.localStorage.setItem(i,e)}catch{}}(class t extends HTMLElement{static{n(`accent-color-selector`,e),r(`accent-color-selector`,t)}connectedCallback(){if(this.dataset.ready===`true`)return;this.dataset.ready=`true`,this.setAttribute(`role`,`radiogroup`),this.setAttribute(`aria-label`,`Accent color`);let e=`accent-color-${s}`;s+=1;for(let t of o){let n=document.createElement(`label`);n.dataset.option=t.value,n.style.setProperty(`--swatch-color`,t.color),n.setAttribute(`aria-label`,t.label),n.innerHTML=`<input type="radio" name="${e}" value="${t.value}" ${c()===t.value?`checked`:``} /><span data-swatch aria-hidden="true"></span><span data-label>${t.label}</span>`,this.append(n)}this.addEventListener(`change`,this),a.dataset.accent=this.value}disconnectedCallback(){this.removeEventListener(`change`,this)}get value(){return this.querySelector(`input:checked`)?.value||`blue`}handleEvent(e){let t=e.target.closest(`input[type='radio']`);e.type!==`change`||!t||!this.contains(t)||(l(t.value),a.dataset.accent=t.value)}});var u=`@layer components {
